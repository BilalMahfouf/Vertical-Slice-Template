# AviaMind Vet — Frontend

A full-featured Veterinary Practice Management System built with **React 19**, **TypeScript**, and **Vite**. This document is the primary reference for any developer joining the project.
...
AviaMind Vet is a single-page application (SPA) that allows a veterinary clinic to manage:

- **Animals** — patient records with health status tracking
- **Clients** — pet owner records
- **Appointments** — scheduling, rescheduling, and cancellation
- **Visits** — clinical visit records with prescriptions and vaccinations
- **Dashboard** — clinic statistics at a glance
- **Settings** — user profile, clinic info, and language preferences
...
```
features/animals/
├── animal-api.ts           # All API functions + TypeScript types for this domain
├── AnimalPage.tsx          # Top-level page component (route target)
├── AnimalDataTable.tsx     # Data table wired to the API
├── add-update-animal.tsx   # Create / Edit form in a Sheet/Dialog
├── view-animal.tsx         # Read-only detail view
├── use-delete-animal.ts    # useMutation hook for the delete action
└── use-animal-toast.ts     # Maps API error codes to user-friendly toast messages
```
...
| `/create-clinic` | `CreateClinicPage` | Onboarding: create the first clinic |
...
| Path | Component | Description |
|---|---|---|
| `/dashboard` | `DashboardPage` | Clinic statistics overview |
| `/animals` | `AnimalPage` | Animal management table |
| `/clients` | `ClientPage` | Client (pet owner) management table |
| `/appointments` | `AppointmentPage` | Appointment scheduling and management |
| `/visits` | `VisitPage` | Clinical visit records |
| `/settings` | `SettingPage` | User profile, clinic info, language |
...
export const animalApi = {
  getAll: async (request: TableRequest): Promise<PagedList<Animal>> => {
...
Displays four statistics cards fetched from `GET /dashboard/cards`:
- Total Animals, Total Appointments, Total Visits, Completed Appointments

Also provides a quick-action button to open the "Add Visit" dialog directly.

### Animals (`src/features/animals/`)

Server-side paginated table of animal records. Each row supports View, Edit, and Delete. Fields include: name, species, breed, gender, birth date, color, microchip number, and status (Active / UnderTreatment / Recovered / Critical / Deceased).

### Clients (`src/features/clients/`)

Manages pet owner records with the same table + CRUD pattern. Each client can have multiple animals.

### Appointments (`src/features/appointments/`)

Manages clinic appointments with additional actions:
- **Reschedule** — updates the appointment date
- **Cancel** — sets status to Cancelled with optional notes

Statuses: Confirmed, Cancelled, Completed, Rescheduled.

### Visits (`src/features/visits/`)

Clinical visit records. Each visit can have associated **Prescriptions** and **Vaccinations** managed through sub-dialogs within the visit detail view.

### Prescriptions (`src/features/prescriptions/`)

Read-only prescription viewer displayed as a dialog within a visit record.

### Vaccinations (`src/features/vaccinations/`)

CRUD table for vaccinations linked to an animal's visit record.

### Settings (`src/features/settings/`)

Three-section page with tabs:
1. **User Profile** — name, email, change password
2. **Clinic** — clinic name, address, phone, email
3. **Language** — switch between English, French, Arabic (triggers RTL layout for Arabic)

Schemas live in the co-located `schemas.ts` file.

### Clinics (`src/features/clinics/`)

Onboarding-only page shown after registration so the user can create their clinic before accessing the main app.
...
const { data, isLoading } = useQuery({
  queryKey: ['animals', tableRequest],
  queryFn: () => animalApi.getAll(tableRequest),
});
...
t(i18nKeyContainer.animals);  // ✅ type-safe
t('animals');                  // ❌ avoid — prone to typos
```
...
interface ProblemDetails {
  type: string;
  title: string;   // error code e.g. "Client.ClientNotFound"
  status: number;
...
export function useAnimalToast() {
  ...
  return {
    onError: (error: unknown) => {
      ...
      if (parsed.code === ErrorCodes.ANIMAL_NOT_FOUND) {
        toast.error(t(i18nKeyContainer.animalNotFound));
      } else {

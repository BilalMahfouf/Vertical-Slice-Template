...
export default function ViewUser({ open, onClose, userId }: ViewUserProps) {
  ...
  const statusLabel = user?.isActive
    ? t(i18nKeyContainer.settingsPage.clinic.staff.active)
    : t(i18nKeyContainer.settingsPage.clinic.staff.inactive);
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h2 className="text-xl font-semibold text-slate-900">
                  {t(i18nKeyContainer.settingsPage.clinic.staff.title)}
                </h2>
                <p className="text-sm text-slate-500 mt-0.5">
                  {t(i18nKeyContainer.settingsPage.clinic.staff.description)}
                </p>
              </div>
            </div>
          </div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-slate-700">
                      {t(i18nKeyContainer.settingsPage.clinic.staff.role)}
                    </label>
                    <div className="flex items-center gap-2 p-3 rounded-lg bg-slate-50 border border-slate-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-slate-700">
                      {t(i18nKeyContainer.settingsPage.clinic.staff.status)}
                    </label>
                    <div className="flex items-center gap-2 p-3 rounded-lg bg-slate-50 border border-slate-200">
                      <StatusBadge
                        status={statusLabel}
